...
test: ## run tests quickly with the default Python
	python -m pytest --basetemp=${ENVTMPDIR} --cov=datareactor --cov-report xml

.PHONY: lint
lint: ## check style with flake8 and isort
	flake8 datareactor tests
	isort -c --recursive datareactor tests
...
fix-lint: ## fix lint issues using autoflake, autopep8, and isort
	find datareactor tests -name '*.py' | xargs autoflake --in-place --remove-all-unused-imports --remove-unused-variables
	autopep8 --in-place --recursive --aggressive datareactor tests
	isort --apply --atomic --recursive datareactor tests

.PHONY: coverage
coverage: ## check code coverage quickly with the default Python
	coverage run --source datareactor -m pytest
	coverage report -m
	coverage html
...
docs: clean-docs ## generate Sphinx HTML documentation, including API docs
	sphinx-apidoc -M --separate --no-toc -o docs/api/ datareactor
	$(MAKE) -C docs html
